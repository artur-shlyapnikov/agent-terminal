#!/bin/sh
# AgentTerminal — Codex session-report shim (architecture §3.10/§3.16/§3.17).
#
# Wired as a Codex `notify` handler (`codex notify = ["<this script>", ...]`).
# The CLI passes one JSON payload argument describing the notification (e.g.
# agent-turn-complete). The shim maps it to a control-protocol report and
# forwards it via `agentctl integration report` NDJSON over the app's unix
# socket, capturing thread/session identity when the payload carries it.
#
...
#                                     of connecting (validator self-test)
#
...
json_string_field() {
    ...
    # so a key inside e.g. {"item":{"type":...}} can never shadow the real
    # field; what remains holds only top-level pairs, and the key must sit
    # right after an outer-object boundary ("{" or ",").
    local text
    text="$2"
    while :; do
        case "$text" in
...
notify_type="$(json_string_field type "$payload")"
thread_id="$(json_string_field thread-id "$payload")"
[ -n "$thread_id" ] || thread_id="$(json_string_field thread_id "$payload")"
[ -n "$thread_id" ] || thread_id="$(json_string_field session_id "$payload")"

# --- map notification to lifecycle tag ----------------------------------------
lifecycle=""
case "$notify_type" in
    agent-turn-complete) lifecycle="idle" ;;
    turn-started)        lifecycle="working" ;;
    *)                   lifecycle="" ;;
esac
...
if [ -n "$thread_id" ]; then
    # Known-notify events carry BOTH lifecycle and thread identity in the
    # SAME report (§3.10): the id must never be dropped just because a
    # lifecycle tag was mapped.
    params="$params,\"sessionReference\":{\"agentKind\":\"codex\",\"opaquePayload\":\"$(json_escape "$thread_id")\",\"capturedAtRevision\":0}"
fi
...
session_ref=""
if [ -n "$thread_id" ]; then
    session_ref="{\"agentKind\":\"codex\",\"opaquePayload\":\"$(json_escape "$thread_id")\",\"capturedAtRevision\":0}"
fi
...
# cannot parse. Positional accumulation is split-proof either way (the
# payload in $1 was captured above, so clobbering positionals is safe).
set -- \
    --agent-id "$AGENT_ID" \
